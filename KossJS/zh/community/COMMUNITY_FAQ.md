...
**A**：**测试覆盖不全，不稳定。** 作者明确将其排除在 ***stable*** 模式之外。除非你非常熟悉 Rust 多线程和 Boa 的 Send/Sync 约束，否则**不要碰**。

### Q7：为什么 Worker 和 FFI 被强制放在 ***stable=False*** 里？
**A**：这是作者的**防御性维护策略**：
1. 这两个功能技术风险高，作者无法兜底。
2. 如果放 ***stable=True***，用户用了出问题会涌来大量无效 Issue（比如“我调了 C 库崩了，快修”）。
3. 放在 ***stable=False*** 是为了明确告诉用户：**用这两个功能代表你是高级用户，出了问题请先审查自己的代码，再来找项目。**
