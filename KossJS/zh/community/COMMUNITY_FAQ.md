...
- **Rust 胶水层（***src/ffi.rs***、***src/runtime.rs*** 等）**：作者看不太懂，可能存在 AI 生成的边界错误，欢迎社区 Rust 开发者帮忙 Review。
...
**A**：作者原话：“不清楚”
以下是社区建议的审查重点：
- ***src/runtime.rs*** 中的 C ABI 导出与内存指针传递（悬垂指针风险）
- Boa 与 Tokio 异步运行时的桥接（死锁/panic 风险）
- 建议在所有 extern "C" 入口函数外层包裹 catch_unwind，防止 Rust panic 跨越 FFI 边界导致宿主崩溃
