...
允许用户注入完全自定义的 FFI 后端实现，替换 SenRi 内置的运行时自动检测机制。

## 概述

默认情况下 SenRi FFI 自动检测当前运行时（KossJS / Bun / Deno / Node.js）并选用对应的内置适配器。但某些场景下你可能需要：

- 使用不同于内置支持的 FFI 引擎（如 `ffi-napi`、`fflate` 等）
- 在不受支持的运行时上使用 SenRi 的上层 API
